...
function pct(value: number | null) {
  return value === null ? "Ã¢â‚¬â€" : `${value.toFixed(1)}%`;
}
...
function SetupStep({ setup, setSetup, onNext }: { setup: Setup; setSetup: (setup: Setup) => void; onNext: () => void }) {
  return (
    <section className="rounded-lg border border-surface-200 bg-white p-6 shadow-sm">
      <h2 className="font-display text-3xl text-navy-800">Step 1 Ã‚ -  Game Setup</h2>
      ...
      <button onClick={onNext} className="button primary mt-6">Begin Stat Entry Ã¢â€ â€™</button>
    </section>
  );
}
  ...
  return (
    <section className="rounded-lg border border-surface-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <h2 className="font-display text-3xl text-navy-800">Step 2 Ã‚ -  Player Stat Entry</h2>
                    ...
                    <span className="absolute left-0 top-full z-10 mt-1 grid w-72 rounded-md border border-surface-200 bg-white p-2 shadow-panel">
                      {suggestions.map((player) => (
                        <button key={player.id} type="button" onClick={() => updateRow(row.id, "playerName", formatPlayerName(player))} className="rounded px-2 py-1 text-left text-sm hover:bg-amber-50">
                          {formatPlayerName(player)} Ã‚ -  {player.position ?? "Position pending"} Ã‚ -  {player.city}
                        </button>
                      ))}
                    </span>
                  ) : null}
                  {column.key === "playerName" && row.isStub ? <small className="mt-1 block text-amber-600">New player Ã¢â‚¬â€ will be verified</small> : null}
                </span>
              ))}
...
function ReviewStep({ setup, homeRows, awayRows, playerOfGame, onSubmit }: { setup: Setup; homeRows: StatRow[]; awayRows: StatRow[]; playerOfGame?: { row: StatRow; score: number }; onSubmit: () => void }) {
  ...
  return (
    <section className="grid gap-5 rounded-lg border border-surface-200 bg-white p-6 shadow-sm">
      <h2 className="font-display text-3xl text-navy-800">Step 3 Ã‚ -  Review and Submit</h2>
      <div className="grid gap-3 rounded-lg bg-surface-100 p-4 md:grid-cols-3">
        ...
        <span><strong className="block text-navy-800">Venue</strong>{setup.venue || "Ã¢â‚¬â€"}</span>
      ...
      <article className="rounded-lg border border-amber-200 bg-amber-50 p-5">
        ...
        <h3 className="mt-2 font-display text-3xl text-navy-800">{playerOfGame?.row.playerName || "Ã¢â‚¬â€"}</h3>
        <p className="text-surface-600">
          {playerOfGame ? `${playerOfGame.row.team} Ã‚ -  ${n(playerOfGame.row.pts)} pts Ã‚ -  ${n(playerOfGame.row.trb)} reb Ã‚ -  ${n(playerOfGame.row.ast)} ast Ã‚ -  Performance Score: ${playerOfGame.score.toFixed(1)}` : "Enter player stats to generate the award preview."}
        </p>
      </article>
