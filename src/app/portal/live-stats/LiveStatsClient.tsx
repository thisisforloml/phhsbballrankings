...
import { createManualStatsSubmission } from "@/app/organizer/live-stats/actions";
...
const statColumns: Array<{ key: keyof StatRow; label: string; width?: string }> = [
  { key: "name", label: "Player name", width: "w-52" },
  { key: "MIN", label: "MIN" },
  { key: "PTS", label: "PTS" },
  { key: "FGM", label: "FGM" },
  { key: "FGA", label: "FGA" },
  ...
  const content = (
      <section className="container-px grid gap-6 pb-16">
        <div className="rounded-lg bg-navy-800 p-6 text-white shadow-panel">
          ...
          <h1 className="mt-3 font-display text-stat-md">Submit One Game for Review</h1>
          <p className="mt-3 max-w-2xl text-navy-200">Manual input creates a submission draft only. Admin review and publishing are required before stats become official.</p>
          <div className="mt-5 flex flex-wrap gap-3">
            {showAdminHome ? <Link href="/admin" className="rounded-md bg-white px-4 py-2 text-sm font-semibold text-navy-800 hover:bg-amber-100">Admin Home</Link> : null}
            <Link href={submissionsHref} className="rounded-md bg-white/10 px-4 py-2 text-sm font-semibold text-white hover:bg-white/20">Back to Submissions</Link>
          </div>
        </div>

        {errorMessage ? <p className="rounded-md bg-red-50 p-4 font-semibold text-red-800">{errorMessage}</p> : null}

        <form action={createManualStatsSubmission} className="grid gap-6">
          ...
          <section className="rounded-lg border border-surface-200 bg-white p-6 shadow-sm">
            <div className="flex flex-wrap items-end justify-between gap-4">
              <div>
                ...
                <h2 className="mt-2 font-display text-3xl text-navy-800">{details.homeTeamName || "Home"} {validation.homeScore} - {validation.awayScore} {details.awayTeamName || "Away"}</h2>
              </div>
          ...
          <TeamStatSection
            ...
            expectedScore={validation.homeScore}
          ...
          <TeamStatSection
            ...
            expectedScore={validation.awayScore}
            gender={details.gender}
            playerSuggestions={playerSuggestions}
          ...
          <section className="rounded-lg border border-surface-200 bg-white p-6 shadow-sm">
            <p className="label">4. Validation Summary</p>
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <ScoreMatch label={details.homeTeamName || "Home team"} points={validation.homeTotals.PTS} expected={validation.homeScore} />
              <ScoreMatch label={details.awayTeamName || "Away team"} points={validation.awayTotals.PTS} expected={validation.awayScore} />
            </div>
            {validation.issues.length ? (
              <div className="mt-4 rounded-md bg-red-50 p-4 text-sm text-red-800">
                <strong className="block">Fix before saving</strong>
                <ul className="mt-2 grid gap-1 list-disc pl-5">{validation.issues.map((issue) => <li key={issue}>{issue}</li>)}</ul>
              </div>
            ) : <p className="mt-4 rounded-md bg-green-50 p-4 text-sm font-semibold text-green-800">Ready to save as a submission draft.</p>}
            {validation.warnings.length ? (
              <div className="mt-4 rounded-md bg-amber-50 p-4 text-sm text-amber-900">
                <strong className="block">Warnings</strong>
                <ul className="mt-2 grid gap-1 list-disc pl-5">{validation.warnings.map((warning) => <li key={warning}>{warning}</li>)}</ul>
              </div>
            ) : null}
            <SubmitButton disabled={validation.issues.length > 0} />
          </section>
        </form>
      </section>
  );
  ...
  return (
    <section className="rounded-lg border border-surface-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="label">3. Player Stats</p>
          <h2 className="mt-2 font-display text-3xl text-navy-800">{title}</h2>
          <p className="mt-1 text-sm text-ink-600">Rows in this section are assigned to {teamName}. New player names will create new player profiles when published.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <button type="button" className="button secondary" onClick={() => onAdd(1)}><Plus className="h-4 w-4" aria-hidden="true" /> {addOneLabel}</button>
          <button type="button" className="button secondary" onClick={() => onAdd(5)}>{addFiveLabel}</button>
        </div>
      </div>

      <div className="mt-5 overflow-x-auto rounded-lg border border-surface-200">
        <table className="w-full min-w-[106rem] text-left text-sm">
          <thead className="bg-surface-100 font-mono text-mono-sm uppercase text-surface-600">
            <tr>
              <th className="px-2 py-2">Starter</th>
              {statColumns.map((column) => <th key={column.key} className="px-2 py-2">{column.label}</th>)}
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id} className="border-t border-surface-200 align-top">
                <td className="px-2 py-2"><input type="checkbox" checked={row.starter} onChange={(event) => onUpdate(row.id, "starter", event.target.checked)} /></td>
                  ...
                  <td key={column.key} className="px-2 py-2">
                    {column.key === "name" ? (
                      ...
                      <input
                        ...
                        className={`${column.width ?? "w-20"} min-h-9 rounded-md border border-surface-200 px-2 py-1`}
            ...
            <tr className="border-t border-navy-200 bg-navy-50 font-semibold text-navy-900">
              <td className="px-2 py-3" colSpan={2}>Team Totals</td>
              <td className="px-2 py-3">{teamTotals.MIN}</td>
              <td className="px-2 py-3">{teamTotals.PTS}</td>
              <td className="px-2 py-3">{teamTotals.FGM}</td>
              <td className="px-2 py-3">{teamTotals.FGA}</td>
              <td className="px-2 py-3">{teamTotals.threePM}</td>
              <td className="px-2 py-3">{teamTotals.threePA}</td>
              <td className="px-2 py-3">{teamTotals.twoPM}</td>
              <td className="px-2 py-3">{teamTotals.twoPA}</td>
              <td className="px-2 py-3">{teamTotals.FTM}</td>
              <td className="px-2 py-3">{teamTotals.FTA}</td>
              <td className="px-2 py-3">{teamTotals.OREB}</td>
              <td className="px-2 py-3">{teamTotals.DREB}</td>
              <td className="px-2 py-3">{teamTotals.TRB}</td>
              <td className="px-2 py-3">{teamTotals.AST}</td>
              <td className="px-2 py-3">{teamTotals.STL}</td>
              <td className="px-2 py-3">{teamTotals.BLK}</td>
              <td className="px-2 py-3">{teamTotals.TOV}</td>
              <td className="px-2 py-3">{teamTotals.PF}</td>
              <td className="px-2 py-3">{teamTotals.FD}</td>
              <td className="px-2 py-3">{teamTotals.plusMinus}</td>
              <td className="px-2 py-3" />
            </tr>
          </tbody>
        </table>
      </div>
      <ScoreMatch label={teamName} points={teamTotals.PTS} expected={expectedScore} />
    </section>
  );
}
...
function PlayerNameInput({ row, gender, suggestions, onChange }: { row: StatRow; gender: "BOYS" | "GIRLS"; suggestions: PlayerSuggestion[]; onChange: (value: string) => void }) {
  ...
  return (
    <span className="relative block">
      <input value={row.name} onChange={(event) => onChange(event.target.value)} className="min-h-9 w-52 rounded-md border border-surface-200 px-2 py-1" placeholder="Full name" />
      {matches.length ? (
        <span className="absolute left-0 top-full z-20 mt-1 grid w-72 rounded-md border border-surface-200 bg-white p-2 shadow-panel">
          {matches.map((player) => (
...
function ScoreMatch({ label, points, expected }: { label: string; points: number; expected: number }) {
  const pass = points === expected && expected > 0;
  return (
    <p className={`mt-4 rounded-md p-3 text-sm font-semibold ${pass ? "bg-green-50 text-green-800" : "bg-red-50 text-red-800"}`}>
...
function SubmitButton({ disabled }: { disabled: boolean }) {
  ...
  return (
    <button type="submit" className="button primary mt-5 w-fit" disabled={disabled || status.pending}>
      {status.pending ? "Saving..." : "Save Manual Submission Draft"}
    </button>
  );
}
