...
export default function PortalPage() {
  ...
  useEffect(() => {
    if (!window.localStorage.getItem(organizerKey)) {
  ...
  if (!ready) {
    return <main className="min-h-screen bg-surface-50 pt-28"><section className="container-px"><div className="skeleton h-40 rounded-lg" /></section></main>;
  }
      ...
      <div className="grid lg:grid-cols-[17rem_1fr]">
        <aside className="bg-navy-800 px-5 py-8 text-white lg:min-h-[calc(100vh-5rem)]">
          ...
          <nav className="mt-8 grid gap-2 font-semibold">
            {["Dashboard", "My Leagues", "Submit Game Stats", "Pending Submissions", "Players", "Account Settings"].map((item) => (
              <a key={item} href="#submit" className="rounded-md px-3 py-2 hover:bg-white/10">{item}</a>
            ))}
          </nav>
        </aside>
        <section className="container-px grid gap-6 py-8">
          <div className="rounded-lg border border-surface-200 bg-white p-6 shadow-panel">
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <a href="#submit" className="button primary">Submit Game Stats</a>
            </div>
            <div className="mt-6 grid gap-3 md:grid-cols-3">
              <Metric value="0" label="Assigned leagues" />
              <Metric value="0" label="Pending submissions" />
              <Metric value="â€”" label="Compliance rate" />
            </div>
          </div>
          ...
          <section id="submit" className="rounded-lg border border-surface-200 bg-white p-6 shadow-panel">
            ...
            {mode === "manual" ? (
              ...
              <div className="mt-6 grid gap-5">
                ...
                {parsing ? <div className="rounded-lg bg-navy-50 p-5 font-mono text-navy-800">Reading statsheetâ€¦ 68%</div> : <StatEntryGrid />}
