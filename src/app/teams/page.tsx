...
import { ageGroups, genders, regions, teams } from "@/lib/mock-data";
...
export default function TeamsPage() {
  ...
  const filtered = useMemo(() => teams
    ...
    .sort((a, b) => b.rating - a.rating), [ageGroup, city, gender, region]);
    ...
    <main className="bg-surface-50 pb-20">
      <section className="bg-[#0F2044] pt-32 text-white">
        <div className="container-px py-14">
          ...
          <p className="mt-4 max-w-2xl text-[#ABABAB]">Verified team ratings based on official submitted games.</p>
        </div>
      </section>
      ...
      <section className="container-px pt-10">
        ...
        {filtered.length ? (
          <div className="overflow-hidden rounded-lg border border-surface-200 bg-white shadow-sm">
            <div className="hidden grid-cols-[5rem_1fr_1fr_10rem_7rem_8rem_5rem_1fr_1fr] gap-3 border-b border-surface-200 px-4 py-3 font-mono text-mono-sm uppercase text-ink-500 lg:grid">
              <span>Rank</span><span>Team</span><span>School</span><span>Location</span><span>Rating</span><span>Record</span><span>PPG</span><span>Top Player</span><span>League</span>
            </div>
            {filtered.map((team, index) => (
              <div key={team.id} className="grid gap-3 border-b border-l-0 border-surface-200 px-4 py-4 transition hover:border-l-[3px] hover:border-l-amber-500 hover:bg-amber-100 last:border-b-0 lg:grid-cols-[5rem_1fr_1fr_10rem_7rem_8rem_5rem_1fr_1fr] lg:items-center">
                ...
                <span className="truncate text-ink-600" title={team.schoolClub}>{team.schoolClub}</span>
                ...
                {team.topPlayer ? <Link className="truncate text-navy-800" title={`${team.topPlayer.firstName} ${team.topPlayer.lastName}`} href={`/players/${team.topPlayer.id}`}>{team.topPlayer.firstName} {team.topPlayer.lastName} · {team.topPlayer.rating}</Link> : <span className="text-ink-400">â€”</span>}
