  ...
  return (
    <div className="grid gap-5">
      ...
      <section className="overflow-hidden rounded-lg border border-surface-200 bg-white shadow-sm">
        <div className="hidden grid-cols-[1.5fr_8rem_8rem_1fr_7rem_7rem_7rem] gap-3 border-b border-surface-200 px-4 py-3 font-mono text-mono-sm uppercase text-ink-500 lg:grid">
          <span>Program</span><span>Abbrev.</span><span>Type</span><span>City / Region</span><span>Teams</span><span>Players</span><span>Games</span>
        </div>
        {filtered.map((program) => (
          <Link key={program.id} href={`/admin/programs/${program.id}`} className="grid gap-2 border-b border-surface-200 px-4 py-4 transition last:border-b-0 hover:bg-navy-50 lg:grid-cols-[1.5fr_8rem_8rem_1fr_7rem_7rem_7rem] lg:items-center">
            <span><strong className="block text-ink-900">{program.fullName}</strong>{program.aliases.length ? <small className="text-ink-500">Aliases: {program.aliases.slice(0, 3).join(", ")}</small> : null}</span>
            ...
            <span className="text-sm text-ink-600">{[program.city, program.region].filter(Boolean).join(", ") || "Not listed"}</span>
            <span className="font-display text-stat-sm text-navy-800">{program.linkedTeamCount}</span>
