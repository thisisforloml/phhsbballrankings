              ...
              <div className="mt-5 grid gap-3">
                ...
                <div className="flex flex-wrap gap-2" aria-label="Record filters">
                  {[
                    ...
                    { value: "NEEDS_CLEANUP" as const, label: `Needs cleanup (${sameContextDuplicateCount})` }
                  ].map((filter) => (
            ...
            {recordFilter !== "INACTIVE" ? (
              <section className="rounded-lg border border-surface-200 bg-white p-5 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <h2 className="font-display text-3xl text-navy-800">{recordFilter === "NEEDS_CLEANUP" ? "Needs Cleanup" : "Active Records"}</h2>
                  ...
                  <span className={`rounded-full px-4 py-2 font-mono text-mono-sm uppercase ${sameContextDuplicateCount ? "bg-amber-50 text-amber-800" : "bg-green-50 text-green-800"}`}>{sameContextDuplicateCount ? `${sameContextDuplicateCount} need cleanup` : "No active same-context duplicates"}</span>
                </div>
