...
function QualityBar({ value }: { value: number }) {
  ...
  return (
    <div ref={ref} className="mt-6 h-2 overflow-hidden rounded-full bg-surface-100">
      <motion.span initial={{ width: 0 }} animate={{ width: inView ? `${value}%` : 0 }} transition={{ duration: 0.8 }} className="block h-full rounded-full bg-navy-800" />
    </div>
  );
}

export function LeagueGrid({ leagues }: { leagues: League[] }) {
  return (
      ...
      {leagues.map((league) => (
        <Link key={league.id} href={`/leagues/${league.id}`} className="rounded-lg border border-surface-200 bg-white p-6 shadow-sm transition duration-200 hover:-translate-y-[3px] hover:border-navy-800 hover:shadow-navy" style={{ borderTopColor: "#0F2044", borderTopWidth: 4 }}>
          ...
          <div className="mt-6 grid grid-cols-2 gap-3">
            ...
            <span><strong className="block font-display text-stat-sm text-navy-800">{league.gamesPerTeam}</strong><small className="font-mono text-mono-sm text-ink-500">Games per team</small></span>
          </div>
