 ...
 * =============================================================================
 *
 * Archived UAAP Season 88 phased GameStat import (historical reference only).
 *
 * Purpose: Early one-shot load of GameStat rows for the first 10 HS Boys games.
 * Historical usage: Superseded intermediate bootstrap step (completed).
 * Replacement: Official submission import — src/lib/submission-official-import.ts
 *
 * This script does NOT use:
 * - PlayerAlias / src/lib/player-import-identity.ts
 * - GameStat immutability / src/lib/game-stat-import-integrity.ts
 *
 * It upserts on (gameId, playerId) and overwrites existing box scores. Re-running
 * can corrupt historical evidence. Source file scripts/data/uaap-s88-hs-boys-10-games.json
 * is no longer in the repo. See scripts/archive/uaap-s88/README.md.
 */
...
/** Run from repository root. Source JSON is no longer shipped. */
const projectRoot = process.cwd();
const dataFilePath = path.join(projectRoot, "scripts/data/uaap-s88-hs-boys-10-games.json");
